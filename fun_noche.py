...
def edit_fun(f):                #crear txt a partir del .json
...
def histograma(network,f):      #crear histogramas  
...
def func_map(network, date):    #crear mapa
    ...
    m.arcgisimage(service='World_Shaded_Relief',xpixels = 1500, dpi=300, verbose= True)
...
def correo_noche(date):         #enviar correos
    ...
    with open(f"{path}/correo/destinatario.json", "r") as json_file:  data = json.load(json_file)
...
if __name__ == "__main__":
    # func_map('RSNC','20200104')
    correo_problema('20200108')